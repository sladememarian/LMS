...
public class SsoController {
    ...
    private static void printMenu() {
        ...
        System.out.println(
            ConsoleColor.CYAN + "1. " + ConsoleColor.RESET + "View Profile"
        );
        System.out.println(
            ConsoleColor.CYAN + "2. " + ConsoleColor.RESET + "Edit Profile"
        );
        System.out.println(
            ConsoleColor.CYAN + "3. " + ConsoleColor.RESET + "Change Password"
        );
        System.out.println(
            ConsoleColor.CYAN + "4. " + ConsoleColor.RESET + "Theme Settings"
        );
        System.out.println(
            ConsoleColor.RED + "5. " + ConsoleColor.RESET + "Logout"
        );
        System.out.println(
            ConsoleColor.RED + "0. " + ConsoleColor.RESET + "Back"
        );
    }
    ...
    private static boolean handleChoice(
        ...
        switch (choice) {
            case "1":
                System.out.println(
                    ConsoleColor.gray(SsoService.viewProfile(email))
                );
                return true;
            case "2":
            ...
            case "5":
                ...
                ConsoleColor.printSuccess(
                    "Session destroyed. You have been logged out."
                );
                return false;
            default:
    ...
    private static void doChangePassword(Scanner scanner, String email) {
        ...
        try {
            ...
            ConsoleColor.printSuccess(
                "Password updated. A confirmation email is in your inbox."
            );
        } catch (BaseException ex) {
            ConsoleColor.printError(ex.getMessage());
        }
    ...
    private static void doChangeTheme(Scanner scanner, String email) {
        ...
        try {
            ...
            ConsoleColor.printSuccess(
                "Theme set to " + SsoService.getTheme(email) + "."
            );
        } catch (BaseException ex) {
            ConsoleColor.printError(ex.getMessage());
        }
